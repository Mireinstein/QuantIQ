...
Target SmaCrossover::on_bar(const Bar& bar) {
    closes_.push(bar.close.to_double());
    if (closes_.size() < slow_) return Target{0.0, "warming up"};
    ...
    // treating it as a crossing would fabricate a trade out of the warm-up.
    if (!primed_) {
        primed_ = true;
        was_above_ = above;
        return Target{0.0, "warm-up complete"};
    }

    if (above != was_above_) {
        was_above_ = above;
        holding_ = above;
        return Target{above ? 1.0 : 0.0,
                      above ? "fast MA crossed above slow" : "fast MA crossed below slow"};
    }
    return Target{holding_ ? 1.0 : 0.0, "no cross"};
}
...
Target MeanReversion::on_bar(const Bar& bar) {
    closes_.push(bar.close.to_double());
    if (closes_.size() < window_) return Target{0.0, "warming up"};

    const double mean = mean_of_last(closes_, window_);
    const double sd = stdev_of_last(closes_, window_, mean);
    if (sd == 0.0) return Target{holding_ ? 1.0 : 0.0, "no deviation to measure"};

    const double z = (closes_[0] - mean) / sd;

    if (!holding_ && z <= z_entry_) {
        holding_ = true;
        return Target{1.0, "z=" + std::to_string(z) + " below entry"};
    }
    if (holding_ && z >= z_exit_) {
        holding_ = false;
        return Target{0.0, "z=" + std::to_string(z) + " reverted"};
    }
    return Target{holding_ ? 1.0 : 0.0, "holding"};
}
...
Target Momentum::on_bar(const Bar& bar) {
    closes_.push(bar.close.to_double());
    if (closes_.size() <= lookback_) return Target{0.0, "warming up"};

    const double then = closes_[lookback_];
    if (then == 0.0) return Target{holding_ ? 1.0 : 0.0, "no reference price"};
    const double change = closes_[0] / then - 1.0;

    if (!holding_ && change > threshold_) {
        holding_ = true;
        return Target{1.0, "up " + std::to_string(change * 100.0) + "% over lookback"};
    }
    if (holding_ && change < 0.0) {
        holding_ = false;
        return Target{0.0, "momentum turned negative"};
    }
    return Target{holding_ ? 1.0 : 0.0, "holding"};
}
...
Target Breakout::on_bar(const Bar& bar) {
    ...
    if (!highest) return Target{0.0, "warming up"};

    if (!holding_ && close > *highest) {
        holding_ = true;
        return Target{1.0, "closed above " + std::to_string(lookback_) + "-bar high"};
    }
    if (holding_ && close < *lowest) {
        holding_ = false;
        return Target{0.0, "closed below " + std::to_string(lookback_) + "-bar low"};
    }
    return Target{holding_ ? 1.0 : 0.0, "holding"};
}
